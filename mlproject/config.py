...
def get_repo_info(repo_path: str):
    # Verify if the repo path is valid
    if not os.path.isdir(repo_path):
        return "The given path is not a valid directory."

    try:
        # Initialize a repo object from the repository path
    ...
    # Get the latest commit hash for the repository
    latest_commit = repo.git.log("-1", "--format=%H")
