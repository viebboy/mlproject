...
ALL_CONFIGS = {
    ...
    "data_dir": ConfigValue(
        DATA_DIR
    ),  # this is the data dir that contains cifar10 data (will be downloaded)
    ...
    "monitor_metric": ConfigValue(
        Accuracy().name()
    ),  # define the name of monitor metric here
    "monitor_direction": ConfigValue(
        "higher"
    ),  # monitor direction: lower means lower is better, similar for higher
    # epoch index
    "checkpoint_idx": ConfigValue(
        -1
    ),  # index of the checkpoint to load. -1 means the latest checkpoint
    # scheduler
    ...
    "grad_accumulation_step": ConfigValue(5),
    ...
    "retain_metric_objects": ConfigValue(
        True
    ),  # if False, save only the metric values (not obj) in history
    # --------- dataset server config ------------
    # --------------------------------------------
    "use_async_loader": ConfigValue(
        False
    ),  # whether to use dataset_server or Torch's dataloader
    ...
    "use_threading_in_data_loader": ConfigValue(
        False
    ),  # whether to use another thread to communicate with other proc
}
